...
var prefix_url="../services" //for prod
// prefix_url="http://localhost:70/dpresume-react/public/services" //for local
...
function saveData() {
    ...
    $.ajax({
        type: "POST",
        url: prefix_url+'/ServiceDetails.php?saveData=1&jwt='+getToken()
        , data: $.param(sParam)
...
function deleteData() {
    ...
    if (deleteText.substr(0, 1) == "*") {
        ...
        var url = prefix_url+"/ServiceDetails.php?jwt="+getToken()+"&" + $.param(sParam)
...
function checkSession() {
    // $.get(prefix_url+'/ServiceDetails.php?sessionCheck=1", function (res) {
    //     return res.trim();
    // })
...
function refresh(e) {
    ...
    $("#idAdminPanel").load(prefix_url+"/ServiceDetails.php?jwt="+getToken()+"&getDataFile=data&getType=option&toload=txtEditor&search=" + $("#idSearch").val())
...
function manageSlider() {
    ...
    $.get(prefix_url+"/ServiceDetails.php?manageSlider=1&jwt="+getToken(), function (res) {
        var newVal = ""
...
function deleteSlider(cur,filename) {
    $("#" + cur).load(prefix_url+"/ServiceDetails.php?jwt="+getToken()+"&manageSlider=del&filename="+filename, function (res) {
        toastr.info(res)
        $("#mainRow" + cur).html("")
    })
}
function checkLogs() {
    $.get(prefix_url+"/ServiceDetails.php?logging=1&jwt="+getToken(), function (res) {
...
function logoutControl() {
    $.get(prefix_url+"/ServiceDetails.php?logout=1&jwt="+getToken(), function (res) {
        if (res.trim() == 0) {
...
function forgotControl() {
    $("#statusMessage").html("please wait...")
    $.get(prefix_url+"/ServiceDetails.php?forgotPassword=1&jwt="+getToken(), function () {
    }).success(function (res) {
