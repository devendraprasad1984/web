...
//        $host.= explode(',', $_SERVER[$header]);
        ...
        {
            $sName = $this->baseDataPath . $filename;
            $newName = str_replace($main_file_ext, ".del", $sName);
...
//            echo "<br/>path: ".$xPath;
                if (strpos($value, ".del") > 0 || (strpos($value, ".pdf") < 0) && (strpos($value, $main_file_ext) === false && $spath !== "resources") || (strpos($value, ".log") > 0 || $value === "slider")) {
//                echo "skipping: $value";
                    continue;
                }
                $label = str_replace($main_file_ext, "", $value);
                ...
                if ($withFileContent === "yes") {
                    ...
                    if (strpos($value, $main_file_ext) > 0)
...
//                print_r($files);
                        foreach ($files as $k1 => $v1) {
                            if (strpos(strtolower($v1), $main_file_ext) > 0 && $spath === "resources") {
                                if ($searchString !== "") {
                                    if (strpos(strtolower("*$v1"), $searchString) > 0)
                                        $data .= "<option value='$host?fileRead=$xPath/$v1' style='font-size: 9pt;'>*" . str_replace($main_file_ext, "", $v1) . "</option>";
                                } else {
                                    $data .= "<option value='$host?fileRead=$xPath/$v1' style='font-size: 9pt;'>*" . str_replace($main_file_ext, "", $v1) . "</option>";
                                }
                            }
        ...
        public function readFileContents($filePath, $fileName)
        {
            ...
            if (strpos($sName, $main_file_ext) > 0) {
                $absPath = $sName;
            } else {
