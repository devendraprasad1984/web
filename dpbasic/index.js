...
function fnSubDivDisplay(ds) {
    ...
    for (let i in dataSub) {
        ...
        x = dataSub[i];
        let ishtml = (x[0].indexOf('.html') !== -1) ? true : false;
        let ishttp = (x[0].indexOf('http') !== -1) ? true : false;
        if (ishtml || ishttp) {
            ...
            // }, function (failData) {
            //     console.log('sub div load error', failData);
            // });
        } else {
            for (let j in x) {
...
let getFromWeb = function (raw, uri, resolve, reject) {
    ...
    let ishttp = (uri.indexOf('http') !== -1) ? true : false;
    uri = ishttp ? 'proxy.php?url=' + uri : uri;
    let req = new XMLHttpRequest();
    req.onload = function () {
        var data = (ishtml || ishttp) ? this.response : JSON.parse(this.response);
